...
import WEDVIDEO from "../../../assets/Images/eyeVideo.mp4";
// import WEDVIDEO from "../../../assets/WEDVIDEO.webm";
...
const Banner2 = React.memo(() => {
    ...
    useEffect(() => {
        // const temp = document.querySelector('#targetText');
        // if (temp) {
        //     const spans = temp.querySelectorAll(':scope > span');
        //     temp.classList.add('animate');
        //     spans.forEach((span, index) => span.style.animation = `fade-in 1.2s ${index * 3.15}s forwards cubic-bezier(0.11, 0, 0.5, 0)`);
        // }

        // Add SVG piece staggered animation (Web Animations API)
        const svg = document.querySelector('.banner-svg');
        if (!svg) return;

        const parts = Array.from(svg.querySelectorAll('*'));
        const baseDurMs = 2500; // duration per phase
        const staggerMs = 45; // stagger between elements

        let cancelled = false;
        const timeouts = [];
        let currentAnims = [];

        // store random offsets per element so the reverse uses the same values
        const transforms = parts.map(() => ({
            tx: Math.round((Math.random() - 0.5) * 500),
            ty: Math.round((Math.random() - 0.5) * 500),
            r: Math.round((Math.random() - 0.5) * 720)
        }));

        const createForward = (el, i) => {
            const { tx, ty, r } = transforms[i];
            const keyframes = [
                { transform: `translate(0px, 0px) rotate(0deg) scale(1)`, opacity: 1 },
                { transform: `translate(${tx}px, ${ty}px) rotate(${r}deg) scale(0)`, opacity: 0 }
            ];
            return el.animate(keyframes, {
                duration: baseDurMs,
                delay: i * staggerMs,
                fill: 'forwards',
                easing: 'cubic-bezier(.11,0,.5,0)'
            });
        };

        const createReverse = (el, i) => {
            const { tx, ty, r } = transforms[i];
            const keyframes = [
                { transform: `translate(${tx}px, ${ty}px) rotate(${r}deg) scale(0)`, opacity: 0 },
                { transform: `translate(0px, 0px) rotate(0deg) scale(1)`, opacity: 1 }
        ...
        async function runCycle() {
            ...
            currentAnims = parts.map((el, i) => {
                try { return createForward(el, i); } catch (e) { return null; }
            }).filter(Boolean);

            try {
                await Promise.all(currentAnims.map(a => a.finished));
            } catch (e) {
                // finished may reject if cancelled; ignore
            }
            ...
            currentAnims = parts.map((el, i) => {
                try { return createReverse(el, i); } catch (e) { return null; }
            }).filter(Boolean);
            ...
            // wait 3s, then run again
            const t = setTimeout(() => { if (!cancelled) runCycle(); }, 3000);
            timeouts.push(t);
        }
        ...
        const handleEnter = () => currentAnims.forEach(a => { try { a.playbackRate = 0.2 } catch (e) { } });
        const handleLeave = () => currentAnims.forEach(a => { try { a.playbackRate = 1 } catch (e) { } });
        ...
        return () => {
            ...
            currentAnims.forEach(a => { try { a.cancel() } catch (e) { } });
        };
    }, [])
    ...
    useEffect(() => {
        const triggerAnimationIfVisible = () => {
            ...
            // Heading animation
            try {
                const headingEl = document.querySelector('#targetText');
                if (headingEl && !headingEl.dataset.animated) {
                    ...
                    if (rect.top < vh && rect.bottom > 0) {
                        ...
                        spans.forEach((span, index) => {
                            ...
                            span.style.animation = `fade-in 2s ${index * 0.6}s forwards cubic-bezier(0.11, 0, 0.5, 0)`;
                        });
                        headingEl.dataset.animated = '1';
                    }
                }
            } catch (e) {
                // fail silently
        ...
        return () => {
            window.removeEventListener('scroll', triggerAnimationIfVisible);
    ...
    const text = "Insight. Inspiration. Innovation".split("");
    useEffect(() => {
        const checkInView = () => {
            if (!containerRef.current || played) return;
            const rect = containerRef.current.getBoundingClientRect();
            const inView = rect.top < window.innerHeight * 0.85 && rect.bottom > 0;
            if (inView) {
                setPlayed(true); // run once
                // optional: remove listeners after play
        ...
        const onScroll = () => {
            // Use rAF for smoother measuring during scroll
    ...
    useEffect(() => {
        if (!played) return;
        const totalDuration = text.length * 0.1 + 0.5; // delay * letters + duration
        const timer = setTimeout(() => setMoveDown(true), totalDuration * 1000);
        return () => clearTimeout(timer);
    }, [played, text.length]);


    return (
        <div className="relative  w-full h-screen min-h-dvh pt-20  bg-[#000000] overflow-hidden">
            {/* <div className="relative w-full h-screen top-18  bg-[#000000] overflow-hidden"> */}

            {/* <figure className="banner-figure" aria-hidden="true">
                <svg className="banner-svg" x="0px" y="0px" viewBox="0 0 883 105.2" xmlSpace="preserve" overflow="visible">
    ...
    C746,53.3,746,53,746,52.7z" />
            ...
            <div className="relative z-50 w-full h-full flex flex-col justify-center items-center text-white">
                ...
                <MediaWithLoader
                    ...
                    type="video/mp4"
                />
                <div className='min-w-full min-h-[calc(100%+80px)] absolute bg-[#000000a7] -top-20 z-10'></div>

                {/* <h3
                    id="targetText"
                    className="font-PoppinsMedium absolute top-[calc(50%-40px)] right-[50%] -translate-y-1/2  flex text-white text-[36px] xl:text-[60px] justify-center flex-col gap-1 z-20
                    <div className="flex ">
                        <span>Insight</span>
                ...
                {moveDown ? (
                    <div
                        className={`z-[60] absolute fadeInOpacity
            ...
            xl:left-[8%] lg:left-[20%] md:left-[18%]`}
                    ...
                    <div
                        ref={containerRef}
                        className={`animatetext z-[60] absolute 
            ...
            xl:left-[22%] lg:left-[20%] md:left-[18%] ${played ? "seven" : ""}`}
                        style={{ fontFamily: "PoppinsSemiBold" }}
                    >
                        {text.map((char, i) => (
                            <span
