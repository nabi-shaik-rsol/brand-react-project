...
import Timer from "../../assets/icons/Advertising Main@300x.png";
...
const Advertising = () => {
    ...
    useEffect(() => {
        const triggerAnimationIfVisible = () => {
            ...
            if (!animateHeading.current) {
                ...
                if (animatedThird || animatedMain || animatedSecondary) animateHeading.current = true;
            }
    ...
    const services = [
        { title: "Campaign\nConceptuali-\nzation", color: "bg-[#e2496d]" },
        { title: "Creative\nDevelopment\n", color: "bg-[#00aa7d]" },
        { title: "360\u00B0\nAdvertising\nCampaigns", color: "bg-[#761a5b]" },
        { title: "Print & Press\nAdvertising", color: "bg-[#c98e27]" },
        { title: "Outdoor\nAdvertising\n(OOH)", color: "bg-[#d37548]" },
        { title: "TV & Radio\nCommercials", color: "bg-[#008fbf]" },
        { title: "Digital\nAdvertising", color: "bg-[#00974d]" },
        { title: "CSR &\nAwareness\nCampaigns", color: "bg-[#c31c26]" }
    ];
    ...
    return (
        <div className="relative  lg:justify-evenly md:justify-between sm:justify-center w-full xs:items-center items-baseline min-h-screen mx-auto bg-black text-white overflow-hidden   
            ...
            <div className='flex justify-between items-baseline  z-20  '>
                ...
                <div className='flex justify-end py-5 md:pr-0 pr-5 z-20 w-full' >
                    <img src={Timer} className='activations-timer z-20 2xl:w-24 2xl:h-24 lg:w-20 lg:h-20 md:w-[70px] md:h-[70px] sm:w-14 sm:h-14  xs:w-14 xs:h-14  w-14 h-14' alt='...' />
                </div>
            </div>
                    ...
                    <div className="w-full">
                        ...
                        <div className="space-y-6  5xl:text-[32px] 5xl:leading-13  2xl:text-[18px]  lg:text-[12px] xs:text-[10px] text-[12px] xs:leading-[1rem] leading-[1.3rem] text-gray-300  2xl:leading-[1.5rem] tracking-wider md:mb-8 sm:mb-8 mb-10" style={{ fontFamily: "Montserrat-Light" }}>
                            <p id="activationsParagraph">
                                At Brain Wave, advertising is not just about visibility—it's about
                                influence. We craft compelling, insight-driven campaigns that cut
                                through the clutter and connect with audiences across every platform.
                                Whether it's print, TV, outdoor, digital, or a full-fledged 360° campaign,
                                our advertising solutions are designed to elevate your brand, spark
                ...
                <div className="justify-end flex flex-col lg:w-[65%] md:w-[100%] sm:w-[100%] w-[100%] xs:w-[100%] items-start z-10 lg:pl-0 md:pl-0 xs:pl-0 pl-0">
                    ...
                    <div className='w-full flex items-end'>
                        <h3 id="activationsWhatWeDo" className="text-[25px] w-full lg:text-[30px]  xl:text-[35px] 5xl:text-[70px] 5xl:leading-25 md:text-[30px] 2xl:text-[45px] xs:text-[25px] font-light mb-7  tracking-wide 2xl:leading-[45px] xl:leading-[40px] sm:leading-[2rem] xs:leading-[1.5rem] leading-[2rem] subHeadingAnimation  " style={{ fontFamily: "PoppinsLight" }}>
                            <span>Our Advertising Services</span>  <br />
                            <span>Include:</span>
                        </h3>
                    </div>
                        ...
                        >
                            {services.map((service, index) => (
                                <div
                                    key={index}
                                    tabIndex={index}
                                    className={`${service.color} lg:p-4 grid-item  md:p-4 p-2  3xl:h-52 3xl:w-52 2xl:h-44 2xl:w-44 xl:w-36 xl:h-36 md:w-28 sm:w-28 gap-2  md:h-28 sm:h-28 w-[120px] h-[120px]  flex flex-col justify-between transition-all duration-300 hover:scale-105 cursor-pointer`}
